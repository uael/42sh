...
/*   Updated: 2018/02/13 19:21:13 by mcanal           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void glob_match_testux(char const *pat, char const *str, t_bool expected)
{
	t_bool match = glob_match(pat, str, 0); //TODO: flags

	MU_ASSERT(
		match == expected,
		"glob_match return mismatch:\n    [pat='%s', str='%s', test=%d, ctrl:%d]",
...
void test_glob_match()
{
	...
	while (*p) {
		...
		p += strlen(p) + 1;
		glob_match_testux(pat, p, expected);
