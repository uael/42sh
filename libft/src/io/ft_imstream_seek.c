...
inline int		ft_imstream_rewind(t_imstream *self, size_t n)
{
	(void)self;
	(void)n;
	return (ST_ERR(errno = ENIMPL));
}

inline int		ft_imstream_forward(t_imstream *self, size_t n)
{
	(void)self;
	(void)n;
	return (ST_ERR(errno = ENIMPL));
...
inline int		ft_imstream_seek(t_imstream *self, size_t off)
{
	if (self->cur > off)
