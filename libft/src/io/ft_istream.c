...
inline t_bool	ft_istream_open(t_istream *self, char const *filename)
{
	self->kind = ISTREAM_FILE;
	return (ft_ifstream_open(&self->u.file, filename));
}

inline t_bool	ft_istream_mopen(t_istream *self, char const *str)
{
	self->kind = ISTREAM_MEM;
	return (ft_imstream_open(&self->u.mem, str));
}

inline t_bool	ft_istream_mnopen(t_istream *self, char const *str, size_t n)
{
	self->kind = ISTREAM_MEM;
	return (ft_imstream_nopen(&self->u.mem, str, n));
}

inline t_bool	ft_istream_close(t_istream *self)
{
